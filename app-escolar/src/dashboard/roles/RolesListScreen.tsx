// ============================================
// RUTA: src/dashboard/roles/RolesListScreen.tsx
// ============================================
  ...
  <Modal transparent visible={visible} animationType="fade">
    <View className="items-center justify-center flex-1 bg-black/50">
      <View className="w-4/5 p-6 mx-4 border rounded-xl bg-card border-border">
        <Text className="mb-2 text-lg font-bold text-foreground">Eliminar rol</Text>
...
const RolCard = ({rol, onEditar, onEliminar}: {
  rol: Rol; onEditar: () => void; onEliminar: () => void;
}) => (
  <View className="p-4 mx-4 mb-3 border bg-card border-border rounded-xl">
    <Text className="text-base font-bold text-foreground mb-1">{rol.nombre}</Text>
    <Text className="text-sm text-muted-foreground">{rol.descripcion}</Text>
...
export default function RolesListScreen({navigation}: Props) {
  ...
  const cargar = async (pagina: number) => {
    try {
      ...
      const data = await getRoles(pagina, 10);
      setRoles(data);
    } catch (e: any) { setError(e.message || 'Error al cargar roles'); }
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <FlatList
        data={roles} keyExtractor={(item) => item.id_rol.toString()}
        contentContainerStyle={{paddingVertical: 16}}
        ListHeaderComponent={
          <View className="px-4 mb-4">
            <Text className="text-2xl font-bold text-foreground">Roles</Text>
            <Text className="text-sm text-muted-foreground">{roles.length} registros · página {page}</Text>
          </View>
        }
        ListEmptyComponent={<View className="items-center py-16"><Text className="text-muted-foreground">No hay roles registrados.</Text></View>}
        renderItem={({item}) => (
          <RolCard rol={item}
            onEditar={() => navigation.navigate('RolForm', {rol: item})}
            onEliminar={() => { setAEliminar({id: item.id_rol, nombre: item.nombre}); setModalVisible(true); }}
          />
        )}
      />
      <View className="flex-row items-center justify-between px-4 py-3 border-t border-border bg-background">
        ...
        <TouchableOpacity onPress={() => setPage((p) => p + 1)} disabled={roles.length < 10}
          className={`px-4 py-2 rounded-lg ${roles.length < 10 ? 'bg-muted' : 'bg-primary'}`}>
          <Text className={roles.length < 10 ? 'text-muted-foreground' : 'text-primary-foreground'}>Siguiente →</Text>
        </TouchableOpacity>
      </View>
      <TouchableOpacity onPress={() => navigation.navigate('RolForm')}
